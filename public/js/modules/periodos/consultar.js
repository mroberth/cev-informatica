import { initAdminShell } from '/js/modules/admin/common.js';
import { apiClient } from '/js/api/client.js';
import { CevAlert } from '/js/utils/CevAlert.js';

let dataTable = null;

const NOMBRE_REGEX = /^\d{4}-(I|II)$/i;

const inicializarDataTable = () => {
  dataTable = new DataTable('#tablaPeriodos', {
    ajax: {
      url: '/a/periodos/consultar_periodos',
      dataSrc: 'data',
    },
    columns: [
      { data: 'nombre' },
      { data: 'fecha_inicio' },
      { data: 'fecha_fin' },
      {
        data: 'estado',
        render: (data) => {
          const badge = data === 'Activo'
            ? 'bg-success text-white'
            : 'bg-secondary text-white';
          return `<span class="badge ${badge}">${data}</span>`;
        },
      },
      {
        data: null,
        orderable: false,
        render: () => '<button class="btn btn-outline-primary btn-sm"><i class="bi bi-pencil"></i></button>',
      },
    ],
    language: {
      url: '/plugins/datatables/es-ES.json',
    },
    responsive: true,
    autoWidth: false,
    pageLength: 10,
    order: [[0, 'desc']],
    dom: '<"d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3"Bf>rt<"d-flex flex-wrap align-items-center justify-content-between gap-2 pt-3"<"d-inline-flex"i><"d-inline-flex"p>>',
    buttons: [
      {
        extend: 'excelHtml5',
        text: '<i class="bi bi-file-earmark-excel me-1"></i>Excel',
        className: 'btn btn-success',
        titleAttr: 'Exportar a Excel',
        exportOptions: {
          columns: [0, 1, 2, 3]
        }
      },
      {
        extend: 'pdfHtml5',
        text: '<i class="bi bi-file-earmark-pdf me-1"></i>PDF',
        className: 'btn btn-danger',
        titleAttr: 'Exportar a PDF',
        exportOptions: {
          columns: [0, 1, 2, 3]
        }
      }
    ],
    initComplete: function () {
      initEditarPeriodo(dataTable);
    },
  });
};

const initEditarPeriodo = (dataTable) => {
  const modal = document.getElementById('modalEditarPeriodo');
  if (!modal) return;

  const form = document.getElementById('formEditarPeriodo');
  const idInput = document.getElementById('editar_id');
  const nombreInput = document.getElementById('editar_nombre');
  const fechaInicioInput = document.getElementById('editar_fecha_inicio');
  const fechaFinInput = document.getElementById('editar_fecha_fin');
  const estadoInput = document.getElementById('editar_estado');

  const showError = (field, msg) => {
    const errorElement = document.getElementById(`${field.id}Error`);
    if (errorElement) errorElement.textContent = msg;
    field.classList.add('is-invalid');
    field.classList.remove('is-valid');
  };

  const clearError = (field) => {
    const errorElement = document.getElementById(`${field.id}Error`);
    if (errorElement) errorElement.textContent = '';
    field.classList.remove('is-invalid');
    field.classList.add('is-valid');
  };

  function validarNombre() {
    let val = nombreInput.value.trim().toUpperCase();
    nombreInput.value = val;
    if (!val) {
      showError(nombreInput, 'El nombre del período es obligatorio.');
      return false;
    }
    if (!NOMBRE_REGEX.test(val)) {
      showError(nombreInput, 'El formato debe ser AÑO-Semestre (ej: 2026-I, 2026-II).');
      return false;
    }
    clearError(nombreInput);
    return true;
  }

  function validarFechaInicio() {
    if (!fechaInicioInput.value) {
      showError(fechaInicioInput, 'La fecha de inicio es obligatoria.');
      return false;
    }
    clearError(fechaInicioInput);
    return true;
  }

  function validarFechaFin() {
    if (!fechaFinInput.value) {
      showError(fechaFinInput, 'La fecha de fin es obligatoria.');
      return false;
    }
    if (fechaInicioInput.value && fechaFinInput.value <= fechaInicioInput.value) {
      showError(fechaFinInput, 'La fecha de fin debe ser posterior a la fecha de inicio.');
      return false;
    }
    clearError(fechaFinInput);
    return true;
  }

  function validarEstado() {
    if (!estadoInput.value) {
      showError(estadoInput, 'El estado es obligatorio.');
      return false;
    }
    clearError(estadoInput);
    return true;
  }

  nombreInput.addEventListener('input', validarNombre);
  fechaInicioInput.addEventListener('change', () => {
    validarFechaInicio();
    if (fechaFinInput.value) validarFechaFin();
  });
  fechaFinInput.addEventListener('change', validarFechaFin);
  estadoInput.addEventListener('change', validarEstado);

  async function verificarNombre() {
    if (!validarNombre()) return false;
    try {
      const response = await apiClient.get(`a/periodos/verificar_nombre?nombre=${encodeURIComponent(nombreInput.value)}&id_excluir=${idInput.value}`);
      if (response.existe) {
        showError(nombreInput, 'El nombre del período ya existe.');
        return false;
      }
    } catch { /* ignore */ }
    return true;
  }

  dataTable.on('click', 'tbody .btn-outline-primary', async function () {
    const data = dataTable.row(this.closest('tr')).data();

    idInput.value = data.id;
    nombreInput.value = data.nombre;
    fechaInicioInput.value = data.fecha_inicio;
    fechaFinInput.value = data.fecha_fin;
    estadoInput.value = data.estado;

    form.querySelectorAll('.is-invalid, .is-valid').forEach(el => el.classList.remove('is-invalid', 'is-valid'));

    new bootstrap.Modal(modal).show();
  });

  form.addEventListener('submit', async (e) => {
    e.preventDefault();

    const validaciones = [
      validarNombre(),
      validarFechaInicio(),
      validarFechaFin(),
      validarEstado(),
    ];

    if (!validaciones.every(v => v === true)) return;

    const nombreOk = await verificarNombre();
    if (!nombreOk) return;

    const btnSubmit = document.querySelector('button[form="formEditarPeriodo"]');
    const originalText = btnSubmit.innerHTML;
    btnSubmit.disabled = true;
    btnSubmit.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span> Actualizando...';

    try {
      const payload = {
        id: parseInt(idInput.value),
        nombre: nombreInput.value.trim(),
        fecha_inicio: fechaInicioInput.value,
        fecha_fin: fechaFinInput.value,
        estado: estadoInput.value,
      };

      await apiClient.post('a/periodos/actualizar', payload);

      CevAlert.success({
        title: 'Actualización Exitosa',
        text: 'El período académico ha sido actualizado correctamente.',
        confirmButtonText: 'Aceptar',
      });

      bootstrap.Modal.getInstance(modal).hide();
      dataTable.ajax.reload(null, false);
    } catch (error) {
      CevAlert.error({
        title: 'Error al actualizar',
        text: error.message || 'Ocurrió un error inesperado.',
        confirmButtonText: 'Aceptar',
      });
    } finally {
      btnSubmit.disabled = false;
      btnSubmit.innerHTML = originalText;
    }
  });
};

const initConsultarPeriodos = () => {
  initAdminShell();
  inicializarDataTable();
};

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initConsultarPeriodos);
} else {
  initConsultarPeriodos();
}
